...
export function isClipboardTempFile(filePath: string): boolean {
	return path.dirname(filePath) === os.tmpdir() && path.basename(filePath).startsWith("pi-clipboard-");
}
...
export function isScreenshotToolName(toolName: string): boolean {
	return toolName === "take_screenshot" || toolName === "chrome_devtools_take_screenshot" || toolName.endsWith("_take_screenshot");
}
