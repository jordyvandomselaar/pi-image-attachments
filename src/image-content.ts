...
export async function maybeResizeImage(image: ImageContent, resizeImage?: ImageResizer | null): Promise<ImageContent> {
	if (!resizeImage) {
		return image;
	}
	try {
...
export async function loadImageContentFromPath(filePath: string, resizeImage?: ImageResizer | null): Promise<ImageContent | null> {
